...
def run_client(host, port, client_id, msg_rate, max_msgs):
    try:
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        print("Usage: python3 load_test.py <IP:PORT> <client_id> <msg_rate> [max_msgs]")
        sys.exit(1)
    ...
    run_client(host, port, cid, msg_rate, max_msgs)
